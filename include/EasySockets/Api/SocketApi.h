...
namespace es
{
    ...
    concept SocketApi = requires(T t, EndPoint end_point, char* buffer, int len)
    {
        ...
        { t.receive_data(buffer, len) } -> std::same_as<int>;
        { t.receive_data_from(buffer, len, end_point) } -> std::same_as<int>;
        { t.send_data(buffer, len) } -> std::same_as<int>;
        { t.send_data_to(buffer, len, end_point) } -> std::same_as<int>;
    }
