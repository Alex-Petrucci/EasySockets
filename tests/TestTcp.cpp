...
#pragma region IPV4 RESOLVED CLIENT
        ...
        es::Socket resolved_ipv4 = es::Socket::make_connected_tcp({"127.0.0.1", 8080});

        std::cout << "Accepting resolved ipv4 on server\n";
        es::Socket accept_res_ipv4 = server.accept_connection();
        ...
        received = accept_res_ipv4.receive_data(buffer, sizeof(buffer));
...
#pragma region IPV6 RESOLVED CLIENT
        ...
        es::Socket resolved_ipv6 = es::Socket::make_connected_tcp({"::1", 8080});

        std::cout << "Accepting resolved ipv6 on server\n";
        es::Socket accept_res_ipv6 = server.accept_connection();

        std::cout << "Sending data from resolved ipv6\n";
        resolved_ipv6.send_data(data.data(), data.size());

        std::cout << "Recieving resolved ipv6 data on server\n";
        received = accept_res_ipv6.receive_data(buffer, sizeof(buffer));
